...
/**
 *   Несмотря на название, это репозиторий для общения с тестовой БД.
 *   Он позволяет непосредственное обращение к БД.
 */
@Repository
public class JDBCExample
{
    ...
    public boolean insertLog(DBLog log)
    {
        ...
        final String ISERT_SQL = "INSERT INTO LOG (LOGSTRING) VALUES (?)";
        jdbcTemplate.update(new PreparedStatementCreator()
        {
            @Override
            public PreparedStatement createPreparedStatement(Connection connection) throws SQLException
            {
                PreparedStatement preparedStatement = connection.prepareStatement(ISERT_SQL);
